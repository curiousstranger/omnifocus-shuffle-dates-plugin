...
(() => {
  ...
  const action = new PlugIn.Action(function (selection) {
    ...
    inputForm.show(formPrompt, buttonTitle).then((formObject) => {
      ...
      const newRandomDate = () => shuffleLib.getRandomDateBetween(settings.startDate, settings.endDate);

      shuffleLib.shuffleDatesForSelection(selection, settings.shuffleDue, settings.shuffleDefer, settings.shuffleReview, newRandomDate, settings.startDate, settings.endDate);
    });
  });

  // only run the action if there are tasks or projects selected
  action.validate = shuffleLib.validateSelection;
